...
import {
    getTeamMemberOpts,
    updateTeamMemberOpts,
    uploadTeamMemberPhotoOpts,
} from "@/queries/team";
...
function RouteComponent() {
    ...
    const { data: member } = useSuspenseQuery(getTeamMemberOpts(id));

    const updateMut = useMutation(updateTeamMemberOpts(id));
    ...
    const form = useForm({
        ...
        onSubmit: async ({ value }) => {
            try {
                await updateMut.mutateAsync({ id, ...value });

                if (photoFile) {
                    try {
                        await photoMut.mutateAsync({ id, file: photoFile });
                    } catch (e: any) {
                        toast.warning(
                            `Miembro actualizado, pero falló la foto: ${e?.message || ""}`,
                            { closeButton: true },
                        );
                    }
                }

                toast.success("Miembro del equipo actualizado correctamente.", { closeButton: true });
                navigate({ to: "/panel/equipo" });
            } catch (e: any) {
                toast.error(e?.message || "Error al actualizar el miembro del equipo.", { closeButton: true });
            }
        },
    });
    ...
    return (
        <div className="px-6 max-w-3xl">
            <div className="flex items-center justify-between mb-6">
                <h1 className="text-2xl font-bold tracking-tight">Editar miembro del equipo</h1>
                <Button
