...
import {
    getProjectAssociateDetailOpts,
    updateAssociateOpts,
    updateProjectAssociateOpts,
...
function RouteComponent() {
    const { id: projectId, associateId } = Route.useParams();
    const navigate = useNavigate();

    const { data: associate } = useSuspenseQuery(
        getProjectAssociateDetailOpts(projectId, associateId),
    );

    const updateMut = useMutation(updateAssociateOpts(associateId));
    const updateRelMut = useMutation(updateProjectAssociateOpts(projectId));
    ...
    const form = useForm({
        defaultValues: {
            ...
            rfc: associate.rfc ?? '',
            curp: associate.curp ?? '',
            lotNum: associate.lotNum ?? '',
            appleNum: associate.appleNum ?? '',
            pendingPayment: associate.pendingPayment ?? false,
        },
        ...
        onSubmit: async ({ value }) => {
            try {
                ...
                toast.success('Asociado actualizado', { closeButton: true });

                try {
                    await updateRelMut.mutateAsync({
                        associateId,
                    ...
                    toast.warning(
                        `El asociado se actualizó, pero falló al actualizar la relación con el proyecto: ${e?.message || ''}`,
                        { closeButton: true },
                    );
                }
                ...
                toast.error(e?.message || 'Error al actualizar el asociado', {
                    closeButton: true,
                });
            }
        },
                ...
                <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                    <div className="space-y-1.5">
                        ...
                        <p className="text-muted-foreground max-w-lg text-sm">
                            Modifica la información del asociado y su relación
                            con el proyecto.
                        </p>
                    </div>
                    <form.Subscribe
