...
function GeocodeButton({ form }: { form: any }) {
    ...
    const onGeocode = async () => {
        ...
        const address = (form.state.values.location ?? "").trim();
        if (!address) {
            toast.error("Ingresa una ubicación antes de geocodificar", { closeButton: true });
            return;
        }
        setLoading(true);
        try {
            ...
            if (!results.length) {
                toast.error("No se encontraron coordenadas para la ubicación");
                return;
            }
            const loc = results[0].geometry.location;
            form.setFieldValue("lat", loc.lat());
            form.setFieldValue("lon", loc.lng());
            toast.success("Coordenadas actualizadas desde la ubicación", {
                closeButton: true,
            });
        } catch (e: any) {
            toast.error(e?.message || "Error al geocodificar la ubicación", {
                closeButton: true,
            });
        } finally {
            setLoading(false);
...
export function LocationSection({ form }: { form: any }) {
    return (
        <Card>
            <CardHeader>
            ...
            <CardContent className="space-y-5">
                    ...
                    {(field: any) => (
                        <Field>
                            ...
                            <Input
                                ...
                                placeholder="Ej. Salida a Mazatlán, Durango, Dgo."
                            />
                ...
                <MapsAPIProvider>
                            ...
                            typeof lat === "number" && typeof lon === "number" ? (
                                ...
                                <div className="bg-muted/40 flex h-64 w-full items-center justify-center rounded-md border border-dashed">
                                    <p className="text-muted-foreground text-sm">
                                        Ingresa la ubicación y geocodifica para ver el mapa
                                    </p>
                                </div>
                            )
                        }
                    ...
                    <div className="flex items-center justify-between gap-4">
                        <FieldDescription>
                            Geocodifica la ubicación o introduce las coordenadas manualmente.
                        </FieldDescription>
